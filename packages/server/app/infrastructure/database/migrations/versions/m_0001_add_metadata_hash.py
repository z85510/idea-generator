...
from app.infrastructure.cache.metadata_hash import metadata_hash as get_metadata_hash
...
async def apply(connection: "aiosqlite.Connection") -> None:
    """Add metadata_hash cache column and unique index. Safe on fresh and existing databases."""
    ...
    if not has_column:
        await connection.execute("ALTER TABLE idea_requests ADD COLUMN metadata_hash TEXT")
        ...
        for row in rows:
            try:
                ...
                await connection.execute(
                    "UPDATE idea_requests SET metadata_hash = ? WHERE id = ?", (h, row[0])
                )
